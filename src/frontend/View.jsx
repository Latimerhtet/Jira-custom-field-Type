...
const View = () => {
  ...
  const { value, valueWithConfig, noValues, noConfiguration } =
    viewComponentText;
  ...
  const summaryAfterProvision = (
    currencySummaryAmount -
    (provision / 100) * currencySummaryAmount
  ).toFixed(2);
      ...
      <Fragment>
        {currencySummaryAmount ? (
          <Fragment>
            ...
            <Text>
              {valueWithConfig} {summaryAfterProvision}{" "}
              {currencySummaryCurrency}
            </Text>
          </Fragment>
        ) : (
          <Text>{noValues}</Text>
